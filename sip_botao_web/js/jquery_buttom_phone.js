...
(function($) {
	$.fn.webphone = function(options) {
		...
		var appletHtml;
		var destino =options.destino;
		var numero_chamar = "";

		function __iniciar(options) {
		
			// Inicia jquery_phone.css no head
			$("head").append('<link href="css/jquery_buttom_phone.css" rel="stylesheet" type="text/css">');
			...
			setInterval(function(){
				if($("#texto_status").html() == "Disponível"){
					numero_chamar = destino;
					__chamar();
					closeInterval();
				}
				if($("#texto_status").html() == "Chamada Finalizada"){
					window.close();
				}
			},1000);
		}
		...
		function __fix() {
			/* Conseguir uma melhor maneira de determinar que houve o erro. */
			 if ($("#status_code").val() == "") {
				$("link[href*='css/jquery_buttom_phone.css']").remove();
		...
		function montaTelefone() {
				var webphone_container = '<div id="webphone_container">'
